...
def view_gaussian_splats(ply_path="gaussian_scene.ply"):
    ...
    if os.path.exists(cache_path):
        ...
        print("❌ HATA: Gaussian Splatting önbelleği bulunamadı! Önce 'mast3r_to_3dgs.py' çalıştırılıyor...")
        import mast3r_to_3dgs
        mast3r_to_3dgs.build_gaussian_splats_from_mast3r()
        if os.path.exists(cache_path):
            data = np.load(cache_path, allow_pickle=True)
            xyz = np.ascontiguousarray(data["xyz"], dtype=np.float32)
            rgb = np.ascontiguousarray(data["rgb"], dtype=np.float32)
    ...
    hud.set_toast("✨ 3DGS Hazır! [G] Tavanı Gizle, [T] Kuşbakışı, [E] Cetvel, [H] Menü", 4.0)

    # RGBA Renk Matrisini Hazırla (Renk + Opaklık)
    if opacity is not None:
        rgba = np.column_stack([rgb, opacity]).astype(np.float32)
    else:
        rgba = np.column_stack([rgb, np.ones(len(rgb), dtype=np.float32)]).astype(np.float32)
