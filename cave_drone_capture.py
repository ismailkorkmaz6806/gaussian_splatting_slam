...
def run_cave_capture(target_keyframes=45):
    ...
    def _on_img(msg):
        try:
            ...
            # ✅ bytes() ile protobuf buffer'ının TAM KOPYASINI al (view değil kopya)
            ...
            if len(arr) == iw * ih * 3:
                ...
                # Beklenen boyut değil (istatistik paketi, header vs.) – kesinlikle yoksay
                return

            # ✅ KARE KALİTE DOĞRULAMASI – bozuk / beyaz / siyah kare filtresi
            # Ortalama parlaklık > 242  → render bozukluğu / overexposed kare → yoksay
            # Ortalama parlaklık < 10   → tam siyah boş frame (compositor bug'ı) → yoksay
            # Standart sapma < 5.0      → neredeyse düz renk kare → yoksay
            mean_val = float(bgr.mean())
            std_val  = float(bgr.std())
            if mean_val > 242.0 or mean_val < 10.0 or std_val < 5.0:
                return  # son geçerli kareyi koru
    ...
    def _on_odom(msg):
        try:
            ...
            drone_pose[3] = math.degrees(pitch)
        except Exception:
            pass
